...
export default async function FollowupQueue() {
  ...
  return (
    <section className="bg-white border border-stone-200 rounded-lg p-5 mb-5">
      ...
      {error && (
        <p className="text-[13px]" style={{ color: "var(--emrg-red)" }}>{error}</p>
      )}
      ...
      <div className="space-y-3">
        {messages.map((m) => (
          <div key={m.code} className="border border-stone-200 rounded-lg overflow-hidden">
            <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 px-4 py-2.5 bg-stone-50 border-b border-stone-100">
              <span className="text-[13.5px] font-semibold text-stone-900">{m.company || m.code}</span>
            ...
            <div className="px-4 py-3">
              <p className="text-[12px] font-semibold text-stone-700 mb-1.5">{m.subject}</p>
              <pre className="text-[12.5px] text-stone-600 whitespace-pre-wrap font-sans leading-relaxed">
                {m.text}
              </pre>
            </div>
          </div>
